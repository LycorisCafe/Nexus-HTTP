...
/**
 * Mark the endpoint method as an HTTP redirect endpoint. The redirect response HTTP status code will {@code 307 Temporary Redirect}. The point is when
 *  a user's API is changed, the user can just annotate it with this and ignore the deep-in-method response processing part (if this annotation presents,
 * the method will not execute). To use this annotation, one of the request method annotations must be presented.
 * <pre>
 *     {@code
 *     @TemporaryRedirect("/redirectEndpoint")
 ...
 * @see <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/307">307 Temporary Redirect (MDN Docs)</a>
 * @since v1.0.0
 */
@Documented
